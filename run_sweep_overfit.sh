...
# 解释器：优先 PY 环境变量 → Linux 的 sam3_test_venv → Windows 侧 cloud_server/.venv
PY="${PY:-python}"
[ -x "$PY" ] || PY="$CLOUD/.venv/Scripts/python.exe"
[ -x "$PY" ] || { echo "✗ 找不到 python，用 PY=/path/to/python bash $0 指定"; exit 1; }
# 项目根由本脚本位置推导，供 configs 里的 ${PROJ_BASE} 展开
export PROJ_BASE="${PROJ_BASE:-$(cd "$HERE/../.." && pwd)}"
cd "$HERE"
