...
class SAM3TrainerNative:
    def __init__(self, config_path, multi_gpu=False):
        ...
        # [#2 扫描] 优先吃本地 base 权重(免 HF 下载/gated 授权)；env SAM3_CKPT 可覆盖，缺失则回退 HF
        _local_ckpt = os.environ.get("SAM3_CKPT",
            os.path.join(os.path.dirname(__file__), "..", "cloud_server", "checkpoints", "sam3.pt"))
        _ckpt = _local_ckpt if os.path.exists(_local_ckpt) else None
        print_rank0(f"  base checkpoint: {_ckpt or '(HF download)'}")
        self.model = build_sam3_image_model(
            device=self.device.type,
            checkpoint_path=_ckpt,          # 非 None 即跳过 HF 下载，加载本地 sam3.pt
            load_from_HF=True,              # checkpoint_path 为 None 时才回退 HF
        ...
        # === [优化 B] 开启梯度检查点 (Gradient Checkpointing) ===
        # 以计算换显存：显存占用可降低 50%，允许 Batch Size 翻倍
        # 尝试通过常见命名查找 image_encoder
        vision_tower = getattr(self.model, "image_encoder", None) or getattr(self.model, "vision_encoder", None)
        if vision_tower:
            print_rank0("🚀 Optimization: Enabling Gradient Checkpointing")
            # 适配不同的 Checkpoint API
            if hasattr(vision_tower, "gradient_checkpointing"):
                vision_tower.gradient_checkpointing = True
            elif hasattr(vision_tower, "set_grad_checkpointing"):
                vision_tower.set_grad_checkpointing(True)
        ...
        self.optimizer = AdamW(
            ...
            fused=True  # 🚀 极限优化 2：开启算子融合，专治 RTX 40 系显卡，提速降显存碎片！
        )
    ...
    def train(self):
        ...
        # config 里的 data_dir 可写 ${PROJ_BASE}/02_数据集/... 或直接写相对项目根的路径，
        # 免得把某台机器的绝对路径钉死在 config 里(换系统/换盘符就失效)。
        _root = os.environ.get(
            "PROJ_BASE",
            os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                         os.pardir, os.pardir)),
        )
        os.environ.setdefault("PROJ_BASE", _root)
        data_dir = os.path.expandvars(os.path.expanduser(self.config["training"]["data_dir"]))
        if not os.path.isabs(data_dir):
            data_dir = os.path.normpath(os.path.join(_root, data_dir))
        ...
        train_loader = DataLoader(
            ...
            # A100 建议至少 8-16 个 worker
            num_workers=self.config["training"].get("num_workers", 8),
            pin_memory=True,
            # === [优化 C] 保持子进程存活，避免每个epoch重建进程的开销 ===
        ...
        for epoch in range(epochs):
            ...
            for i, batch_dict in enumerate(pbar):
                ...
                # === [优化 D-2] 梯度累积核心逻辑 ===
                # 1. Loss 归一化：除以累积步数，防止梯度过大
                loss_to_backward = total_loss / accum_steps

                # 2. 反向传播 (梯度会累积在 .grad 中)
                loss_to_backward.backward()

                # 3. 仅在达到累积步数时更新权重
                if (i + 1) % accum_steps == 0:
                    # 建议：LoRA 微调加上梯度裁剪，防止 Bfloat16 下梯度爆炸
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1.0)

                    self.optimizer.step()
                    self.optimizer.zero_grad(set_to_none=True)

                # Track training loss (记录原始 Loss 供显示)
                # 注意：因为是累积平均，这里直接 append total_loss 即可反应当前 batch 的情况
                current_loss_val = total_loss.item()
                train_losses.append(current_loss_val)
            ...
            if has_validation and val_loader is not None:
                ...
                if is_main_process():
                    ...
                    # [#2 过拟合扫描] 存每-epoch checkpoint，供训练后按"水位线px误差"离线选模型
                    # （val_loss 最低的 epoch ≠ 水位线最准的 epoch；默认关，不影响任何既有训练）
                    if self.config["training"].get("save_every_epoch", False):
                        save_lora_weights(model_to_save, str(out_dir / f"epoch{epoch+1:02d}_lora_weights.pt"))
